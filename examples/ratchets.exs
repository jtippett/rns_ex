...
# keys every time an announce is sent.                   #
##########################################################
...
defmodule Ratchets.Server do
  def run(configpath) do
    ...
    destination_hexhash = RNS.hexrep(echo_destination.hash, false)
    echo_destination = RNS.Destination.enable_ratchets(echo_destination, "/tmp/#{destination_hexhash}.ratchets")
...
defmodule Ratchets.Client do
  def run(destination_hexhash, configpath, timeout) do
    ...
    if RNS.loglevel() < RNS.log_info() do
...
cond do
  opts[:server] ->
    Ratchets.Server.run(opts[:config])

  length(args) > 0 ->
    [destination_hexhash | _] = args
    Ratchets.Client.run(destination_hexhash, opts[:config], opts[:timeout])

  true ->
    IO.puts("")
    IO.puts("Usage: mix run examples/ratchets.exs [-s | --server] [--timeout SECONDS] [--config PATH] [DESTINATION_HASH]")
    IO.puts("")
end
