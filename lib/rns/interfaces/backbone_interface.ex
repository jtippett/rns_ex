defmodule RNS.Interfaces.BackboneInterface do
  ...
  def handle_call(:detach, _from, state) do
    ...
    Enum.each(state.spawned_interfaces, fn pid ->
      if is_pid(pid) and Process.alive?(pid), do: RNS.Interfaces.BackboneClientInterface.stop(pid)
    end)
  ...
  defp handle_new_connection(state, client_socket) do
    ...
    case RNS.Interfaces.Interface.start_child(RNS.Interfaces.BackboneClientInterface, client_opts) do
      {:ok, pid} ->
