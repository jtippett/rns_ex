defmodule RNS.Utilities.RNSD do
  @moduledoc """
  Reticulum Network Stack Daemon.

  Starts a Reticulum instance and runs as a long-lived daemon process.
  This is the Elixir equivalent of the Python `rnsd` utility.

  Can be invoked as an escript (`rnsd`) or called programmatically via
  `RNS.Utilities.RNSD.main/1`.
  ...
  def main(args) do
    case parse_args(args) do
      {:ok, opts} ->
        cond do
          opts.version ->
            IO.puts("rnsd #{RNS.Version.version()}")

          opts.exampleconfig ->
            IO.puts(example_rns_config())
      ...
      true ->
        {:ok,
         %{
           ...
           service: Keyword.get(parsed, :service, false),
  ...
  @doc """
  Initializes Reticulum and runs the daemon loop.
  ...
  @doc """
  Stops a running rnsd daemon by sending a `:stop` message.
  """
  @spec stop(pid()) :: :stop
  def stop(pid) do
    send(pid, :stop)
  ...
  defp print_usage do
    ...
    Options:
      ...
      --exampleconfig    Print verbose example configuration and exit
      --version          Print version and exit
  ...
  @doc """
  Returns a verbose example Reticulum configuration string.

  This matches the example config from the Python `rnsd` utility.
  """
  @spec example_rns_config() :: String.t()
  def example_rns_config do
    """
